...
pub trait RefVisitor {
    fn visit_class(&mut self, class: &Class) -> anyhow::Result<()> {
        walk_class(self, class)
    }

    fn visit_method(&mut self, method: &Method) -> anyhow::Result<()> {
        walk_method(self, method)
    }

    fn visit_field(&mut self, field: &Field) -> anyhow::Result<()> {
        walk_field(self, field)
    }

    fn visit_statement(&mut self, stmt: &Statement) -> anyhow::Result<()> {
        walk_statement(self, stmt)
    }

    fn visit_expression(&mut self, expr: &Expression) -> anyhow::Result<()> {
        walk_expression(self, expr)
    }
}

pub fn walk_class<V: RefVisitor + ?Sized>(v: &mut V, class: &Class) -> anyhow::Result<()> {
...
pub fn walk_field<V: RefVisitor + ?Sized>(v: &mut V, field: &Field) -> anyhow::Result<()> {
    if let Some(expr) = &field.init_value {
        v.visit_expression(expr)?;
    }
    Ok(())
}

pub fn walk_statement<V: RefVisitor + ?Sized>(v: &mut V, stmt: &Statement) -> anyhow::Result<()> {
    match stmt {
...
pub fn walk_expression<V: RefVisitor + ?Sized>(v: &mut V, expr: &Expression) -> anyhow::Result<()> {
    match expr {
        Expression::Constant { .. }
        | Expression::Load { .. }
        | Expression::LoadTemp { .. } => {}
        ...
        Expression::BoolOp { op: _, lhs, rhs } => {
            v.visit_expression(lhs)?;
            v.visit_expression(rhs)?;
        },
